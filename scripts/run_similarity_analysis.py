...
出力:
    ...
    ├── candidate_ranking_{region}.csv (候補地ランキング)
    ├── candidate_locations_{region}.csv (候補地位置情報付きランキング)
    ├── candidate_locations_{region}.kmz (候補地位置KMZファイル)
...
def generate_kml_content(ranked_sites_df, title="Candidate Sites Ranking"):
    ...
    for idx, (_, row) in enumerate(ranked_sites_df.iterrows()):
        ...
        <b>座標:</b> {row['center_lat']:.6f}, {row['center_lon']:.6f}
        ]]>
        """
...
def generate_candidate_location_outputs(candidate_ranking_df, candidate_csv_path, output_dir, region):
    ...
    ranked_sites_with_location.to_csv(location_csv_path, index=False)
